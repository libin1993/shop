...
public class AgeStatisticsActivity extends BaseMvpActivity<AgeStatisticsContact.Presenter> implements AgeStatisticsContact.View {
    ...
    private void loadData() {
        ...
        map.put("chart_types", "guest_age_statistics");
    ...
    @Override
    protected AgeStatisticsContact.Presenter initPresenter() {
        return new AgeStatisticsPresenter();
    }

    @Override
    public void loadSuccess(List<GuestCountBean.DataBean.GuestAgeStatisticsBean> ageList) {
        final List<String> list = new ArrayList<>();
        //设置数据
        List<BarEntry> entries = new ArrayList<>();
        for (int i = 0; i < ageList.size(); i++) {
            entries.add(new BarEntry(i,  ageList.get(i).getCnt()));
            list.add(ageList.get(i).getAge_group());
        }

        XAxis xAxis = chartAge.getXAxis();
        xAxis.setPosition(XAxis.XAxisPosition.BOTTOM);
        xAxis.setLabelCount(ageList.size());
