...
public class SexStatisticsActivity extends BaseMvpActivity<SexStatisticsContact.Presenter> implements SexStatisticsContact.View {
    ...
    public void loadSuccess(List<GuestCountBean.DataBean.GuestGenderStatisticsBean> genderList) {
        ...
        for (int i = 0; i < genderList.size(); i++) {
            entries.add(new BarEntry(i,  genderList.get(i).getCnt()));
            list.add(genderList.get(i).getGender());
        }

        XAxis xAxis = chartAge.getXAxis();
        xAxis.setPosition(XAxis.XAxisPosition.BOTTOM);
        xAxis.setLabelCount(genderList.size());
        ...
        //一个LineDataSet就是一条线
        BarDataSet barDataSet = new BarDataSet(entries, "性别");
        //格式化显示数据
